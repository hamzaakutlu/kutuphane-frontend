...
const AdminAuthors = () => {
  ...
  const handleAddAuthor = async (e) => {
    ...
    try {
      await api.post('/author', newAuthor);
      setNewAuthor({ name: '' });
      loadAuthors();
    } catch (err) { alert("Yazar eklenirken hata oluştu."); }
  };
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.header}>✍️ Yazar Yönetimi</h1>
      
      <div style={styles.card}>
        <form onSubmit={handleAddAuthor} style={styles.formLine}>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Yazar Adı Soyadı</label>
            <input 
              style={styles.input} 
      ...
      <div style={styles.card}>
        <table style={styles.table}>
          <thead>
            <tr>
              <th style={styles.th}>YAZAR ADI</th>
              <th style={{...styles.th, textAlign: 'center'}}>TOPLAM KİTAP</th>
              <th style={{...styles.th, textAlign: 'right'}}>İŞLEMLER</th>
            </tr>
          </thead>
          <tbody>
            {authors.map(author => (
              <tr key={author.id} style={{ transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#fafafa'} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                <td style={{...styles.td, fontWeight: '600'}}>{author.name}</td>
                ...
                <td style={{...styles.td, textAlign: 'right'}}>
                  <button 
                    ...
                    onClick={() => { if(window.confirm('Bu yazarı silmek istediğinize emin misiniz?')) api.delete(`/author/${author.id}`).then(loadAuthors) }}
                  >
                    SİL
                  </button>
                </td>
